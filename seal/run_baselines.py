...
Output files (all written to ./seallogs/):
    ...
    mistral_judge_summary.json            ← Gemini vs Mistral supplementary
...
Quota notes:
    - Reflexion + ZeroShot: HF token only (Qwen via agent.py). No Gemini calls.
    - NO_RUBRIC_EVOLUTION: Gemini calls (evaluate only, no evolve). ~50-100 calls.
    - Mistral judge: HF token only for judge. No Gemini calls.
...
def run_reflexion(total: int = TOTAL_SCENARIOS) -> None:
    ...
    No judge calls — Gemini quota untouched.
    """
...
def run_no_rubric_evolution(total: int = TOTAL_SCENARIOS) -> None:
    ...
    Uses Gemini (evaluate calls only, no evolve). ~50-150 Gemini calls total.
    """
    ...
    for sid in range(total):
        ...
        try:
            ...
            print(f"  {task_key}: SKIP — {e}")
        time.sleep(3)  # Gemini rate limit buffer

    _write("no_rubric_evolution_summary.json", all_results)
    print(f"NO_RUBRIC_EVOLUTION complete. {len(all_results)}/{total} tasks. "
          f"Total Gemini calls: {request_count}")


# ── Condition 4: Gemini vs Mistral judge ablation ────────────────────────────
...
def run_mistral_judge(total: int = TOTAL_SCENARIOS) -> None:
    ...
    No Gemini calls. Uses HF_TOKEN for both agent (Qwen) and judge (Mistral).
    ...
    """
    print(f"\n=== Gemini vs Mistral Judge Ablation ({total} tasks) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mistral_judge",action="store_true", help="Gemini vs Mistral judge ablation")
