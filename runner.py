...
class KeyRotator:
    ...
    def __init__(self, keys: list):
        if not keys:
            raise ValueError(
                "No Groq API keys found. Add keys.json or set GROQ_API_KEY env var."
            )
    ...
    def tick(self):
        ...
        if self.calls_on_current_key >= KEY_CALL_CEILING:
            if self.index + 1 >= len(self.keys):
                raise RuntimeError(
                    f"[KEY EXHAUSTED] All {len(self.keys)} keys have hit "
                    f"their ceiling ({KEY_CALL_CEILING} calls each). "
                    f"Rotate in new keys or wait 24h."
                )
            ...
            print(
                f"[KEY ROTATED] Switched to key index {self.index} "
    ...
    def force_rotate(self, reason: str = ""):
        ...
        print(
            f"[KEY FORCE-ROTATED] {reason} -> switched to key index "
            f"{self.index} (key ...{self.keys[self.index][-6:]})"
        )
...
class SEALRunner:
    def __init__(
        self,
        condition: str = "SEAL_FULL",
        output_dir: str = "./seallogs",
    ...
    def run_task_lifecycle(
        ...
        print(
            f"Running task {task_id} under "
            f"[{self.condition}] condition..."
        )
        ...
        for iteration in range(1, 4):
            ...
            raw_failure_enum = getattr(
                evaluation_report,
                "failure_type",
                None,
            )

            extracted_failure_str = (
                raw_failure_enum.value
                if raw_failure_enum
                else "none"
            )
            ...
            result = TaskResult(
                ...
                rubric_hash=make_rubric_hash(
                    rubric_string_representation
                ),
...
def run_comprehensive_suite(max_calls: int = 200):
    ...
    print(
        "=== Launching SEAL Runner Production Benchmark ==="
    )

    print(
        f"Keys loaded: {len(rotator.keys)} | "
        f"Ceiling per key: {KEY_CALL_CEILING} | "
    ...
    for sid in range(total_scenarios):
        ...
        if rotator.total_calls >= max_calls:
            print(
                f"[STOPPED] Hit max_calls budget ({max_calls}) "
                f"at scenario {sid}. "
        ...
        try:
            ...
            print(
                f"  {task_key}: {calls} calls | "
                f"key status: {rotator.status()}"
            )
