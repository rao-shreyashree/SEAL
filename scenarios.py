"""
scenarios.py — ALFWorld simulation environment for SEAL.

Each scenario has a forced_outcome that controls the environment's behavior.
Optional 'drift_item' field overrides the hardcoded GOAL_DRIFT token in agent.py.
"""
...
class MultiScenarioALFWorldEnv:
    def __init__(self, scenario_id=0):
        self.scenarios = [
            ...
            {   # drift_item overrides the 'key ring' hardcode in agent.py — add to any GOAL_DRIFT scenario
                "goal": "Put a tissue box on the nightstand.",
                "target": "nightstand", "item": "tissue",
                "forced_outcome": "GOAL_DRIFT",
                "drift_item": "key ring",   # what the agent will mistakenly try to place
            },
    ...
    def set_scenario(self, scenario_id):
        ...
        self.steps = 0
        self.state_index = 0
        # Expose drift_item as attribute so agent.py can read it without env.data coupling
    ...
    def step(self, action):
        ...
        if "put" in act or "place" in act or "examine" in act:
            if outcome == "GOAL_DRIFT" and self.drift_item.split()[0] in act:
